...
# )
train_loader = Mnist(batch_size).train()
test_loader = Mnist(batch_size).test()
...
EbeddingNet.cuda(device="cuda:1")

# ArcSoftLoss = Net.ArcMarginProduct()
ArcSoftLoss = Net.Arcface()
ArcSoftLoss.cuda(device="cuda:1")
...
def visualize(feat, labels, epoch):
    ...
    timg = tt(img)
    timg.unsqueeze(0)
    writer.add_image('ArcSoft', timg, epoch)
...
def visualize_cos(feat, labels, epoch):
    ...
    weight = ArcSoftLoss.state_dict()['weight'].t()
    ...
    timg = tt(img)
    timg.unsqueeze(0)
    writer.add_image('ArcSoft_cos', timg, epoch)
...
def train_mnist(epoch, optimizer_nn):
    ...
    for images, label in train_loader:
        images = images.cuda(device="cuda:1")
        label = label.cuda(device="cuda:1")
        # 前向传播
        feature= EbeddingNet(images, epoch)

        feature_list.append(feature)
        label_list.append(label)
        # loss,out = ArcSoftLoss(feature, label)
        loss, out = ArcSoftLoss(feature, label)
        # 反向传播
...
def main():
    ...
    optimizer_nn = optim.Adam([
        ...
        {'params': ArcSoftLoss.parameters(), 'lr': 1e-3},
    ])
    milestone = [20,40,60]
    for epoch in tqdm(range(num_epoches)):
        ...
        print(train_loss_nn/len(train_loader))
    ...
    #     os.mkdir('./save')
    # os.makedirs('./save_insight',exist_ok=True)
    # torch.save(EbeddingNet, './save_insight/ArcSoft.-{}-pkl'.format(num_epoches))
